...
my_stopwords_list = stopwords.words('english') + stopwords.words('french')
my_stopwords_list.extend('op pp ch dun dune ibid cf iv xvi xix xviii xix'.split())
...
def tfidfPublications(pathToMemex):
    ...
    vectorizer = CountVectorizer(ngram_range=(1,1), min_df=1, max_df=0.75, stop_words=my_stopwords_list)
    ...
    pathToSave = os.path.join(pathToMemex, "results_tfidf.json")
    ...
    pathToSave = os.path.join(pathToMemex, "results_cosineDist.json")
    with open(pathToSave, 'w', encoding='utf8') as f9:
        json.dump(tfidfTableDicFilt, f9, sort_keys=True, indent=4, ensure_ascii=False)
